...
class plottData:
	...
	def prepration(self, fileName):
		...
		importlib.reload(classImportLA)
		...
		fileNameArr = []
		fileNameArr.append(str("500f80271400-1.txt"))
		...
		if int(channelPlotterBool) == 0 or int(channelPlotterBool) == 2:
			LA = classImportLA.learningAlgs()
			for i in range(len(fileNameArr)):
				#absoluteAdd = "/home/netlab/Desktop/thesis/data/500f80271400/"
				fileAbsoluteAdd = str("500f80271400-1.txt")
				...
				if int(plotterBool) == 1:
					...
					# time.sleep(1)
					self.plotting(fileNameArr[i], chanUtil, timeArr, address, channelPlotterBool)
				# else:
					# return chanUtil, timeArr
	...
	def plotting(self, fileName, CU, Time, address, channelPlotterBool):
		...
		offline.plot(fig, filename = plotName, image='svg')
